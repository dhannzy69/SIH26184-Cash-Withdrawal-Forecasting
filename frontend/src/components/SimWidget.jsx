...
import { Play, Zap, ArrowRight, CheckCircle, RefreshCw } from 'lucide-react';

export default function SimWidget({ caseId, onTransactionInjected, isSimulating }) {
  const [amount, setAmount] = useState(35000);
  const [receiverRegion, setReceiverRegion] = useState('Mumbai');
  const [receiverAcc, setReceiverAcc] = useState('A_MULE_NEW');
  ...
  const handleSimulate = async (e) => {
    ...
    const payload = {
      case_id: caseId,
      sender_account: 'A_TERMINAL_MULE',
